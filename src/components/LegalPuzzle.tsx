...
const puzzles = [
  ...
  },
  {
    ...
    options: ['باطل است', 'غیر نافذ است', 'صحیح است', 'موقوف به اجازه موکل است'],
  ...
  if (current >= puzzles.length) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 text-center">
        <h2 className="text-2xl font-bold text-green-600 mb-4">🎉 همه معماها حل شد!</h2>
          ...
          {puzzle.options.map((opt) => (
            <label
              ...
              className={`p-2 border rounded cursor-pointer ${
                selected === opt ? 'bg-yellow-200 border-yellow-600' : 'bg-gray-100'
              }`}
            >
              <input
                type="radio"
